...
ALLOWED_USERS = [964098780557373550, 490600486794166308]  # You + Other ID
...
def calculate_oil_summary(messages):
    ...
    for i in range(len(messages) - 1):
        try:
            after1 = float(messages[i].content.split("Oil stock after :")[1].strip())
            before2 = float(messages[i + 1].content.split("Oil stock before :")[1].strip())
            diff = before2 - after1
            if diff > 0:
                total_taken += diff
        except Exception:
...
async def oil_summary(ctx, start: str, end: str):
    ...
    try:
        ...
        await asyncio.sleep(2)  # Allow edited messages to be fetched
        messages = [msg async for msg in channel.history(after=start_time, before=end_time)]
...
async def bonus_summary(ctx, start: str, end: str):
    ...
    try:
        ...
        trip_counts = calculate_trip_summary(messages)
        if not trip_counts:
            return await ctx.send("No trips found in the given time range.")
...
async def final_calc(ctx, start: str, end: str):
    ...
    try:
        ...
        p.setFont("Helvetica", 12)
        p.drawString(60, y, f"Total Oil Taken: {total_oil:.2f} Liters")
        y -= 20
        ...
        y -= 20
        p.drawString(60, y, f"Total Bonus: {total_bonus_amount:,} units")
        y -= 20
